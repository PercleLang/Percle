...
class AstNodeBase {
    public:
	    ...
	    void setInput(Namespace nsIn, bool dynamicIn, Type left, Type right) {
			if (inputHasBeenSet) {
				throw PercleError("Tried to set input on an AST node '"+getString()+"' more then once", INTERNAL_ERROR, getToken());
			}
	...
	virtual AstNode makeCopy(bool copyCache)=0;
	...
	Type getReturnType() {
		if (!returnType) {
			if (!inputHasBeenSet) {
				throw PercleError("Tried to get return type from AST node when input had not been set", INTERNAL_ERROR, getToken());
			}
		...
		if (!returnType) {
			throw PercleError("AST Node "+getString()+" failed to supply a return type", INTERNAL_ERROR);
		}
	...
	Action getAction() {
		if (!action) {
			if (!ns) {
				throw PercleError("Tried to get action from AST Node when input had not been set", INTERNAL_ERROR, getToken());
			}
	...
	void dealWithConstants() {
		if (!ns) {
			throw PercleError("tried to deal with constants before input was set", INTERNAL_ERROR, getToken());
		}
